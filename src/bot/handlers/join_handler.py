...
from bot.utils import ERROR_MSG, get_display_name, h
    ...
    try:
        async with get_db() as db:
            ...
            telegram_group_member_ids = await _get_telegram_group_member_ids(
                context.bot, telegram_chat_id, telegram_user.id
            )
            ...
            user_display_name = get_display_name(user_data)
            group_name = h(group["name"])
            success_msg = (
                f"<b>Successfully joined {group_name}!</b>\n\n"
            ...
            group_notification = (
                f"<b>{user_display_name}</b> has joined the expense group <b>{group_name}</b>!"
            )
            try:
                await context.bot.send_message(
                    chat_id=telegram_chat_id, text=group_notification, parse_mode="HTML"
                )
...
async def _get_telegram_group_member_ids(
    bot, telegram_chat_id: int, user_id: int
) -> list[int] | None:
    """
    ...
    try:
        ...
        try:
            member_info = await bot.get_chat_member(chat_id=telegram_chat_id, user_id=user_id)

            # Check if user is actually a member (not left/kicked/restricted)
            if member_info.status in ["left", "kicked"]:
                logger.warning(
                    f"User {user_id} is not a member of Telegram group {telegram_chat_id}"
                )
                return None
