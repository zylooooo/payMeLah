...
from bot.utils import ERROR_MSG, get_display_name, h, validate_chat_type
from infrastructure import get_db
from models import CATEGORY_DISPLAY, ExpenseCategory
from services import ExpenseService, GroupService, UserService
from shared import ExpenseNotFoundException, GroupNotFoundException, UnauthorizedActionException
...
def _format_expense_list_item(expense: dict, index: int, payer_name: Optional[str] = None) -> str:
    ...
    # Truncate description if too long, then escape (truncate first so we
    # never split an HTML entity produced by escaping)
...
def _format_expense_details(
    expense: dict, participants: list, payer_name: Optional[str] = None
) -> str:
    """Format full expense details with payer info and proper date formatting."""
    description = h(expense.get("description")) or "No description"
    amount = expense.get("amount", 0)
    currency = expense.get("currency", "SGD")
    expense_date = expense.get("expense_date", "")
    ...
    try:
        ...
        if len(groups) == 1:
            ...
            await update.message.reply_text(
                "<b>Select a group to view expenses:</b>", parse_mode="HTML", reply_markup=keyboard
            )
...
async def _show_expense_list(
    ...
    try:
        ...
        message = _format_expense_list_message(
            expenses, group_name, payer_names, page, total_count, active_filter
        )
    ...
    try:
        async with get_db() as db:
            expense_data = await ExpenseService.get_expense_with_participants(
                db, expense_id, requesting_user_id=telegram_user.id
            )
            ...
            can_edit, _ = await ExpenseService.can_modify_expense(db, expense_id, telegram_user.id)
            can_delete, _ = await ExpenseService.can_delete_expense(
                db, expense_id, telegram_user.id
            )

        message = _format_expense_details(
            expense_data, expense_data.get("participants", []), payer_name
        )
        group_id = expense_data.get("group_id")
        keyboard = ExpenseKeyboard.get_expense_details_keyboard(
            expense_id=expense_id, group_id=group_id, can_edit=can_edit, can_delete=can_delete
        )
...
async def handle_expense_view_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
            ...
            try:
                ...
                if not groups:
                    await query.edit_message_text(
                        "Groups list expired. Please use /expenses again."
                    )
                    return
    ...
    if action == ExpenseKeyboard.ACTION_CONFIRM_DELETE:
        ...
        try:
            ...
            await query.edit_message_text("Expense deleted successfully.", reply_markup=keyboard)
