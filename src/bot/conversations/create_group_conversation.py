...
def _format_field_prompt(field_name: str, field_label: str) -> str:
    ...
    return f"<b>{field_label}</b>\n\nPlease enter your {field_label.lower()}:"


def _format_summary(group_data: Dict[str, Any]) -> str:
    """Format the summary of group creation."""
    name = h(group_data.get("name", "Not set"))
    description = h(group_data.get("description")) or "Not set"
...
async def _show_field_prompt(
    ...
    keyboard = GroupKeyboard.get_navigation_keyboard(
        field_name, is_first=is_first, show_skip=show_skip
    )
...
async def _show_summary(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await _send_or_edit_message(
        update, context, message, GroupKeyboard.get_confirm_keyboard("create", 0)
    )
...
async def start_create_group(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    logger.info(
        f"Starting create group conversation for user: {telegram_user.id} @{telegram_user.username}"
    )
    ...
    try:
        ...
        logger.error(
            f"Error auto-creating user account for user {telegram_user.id}: {e}", exc_info=True
        )
...
async def handle_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not is_valid:
        logger.warning(
            f"Invalid group name input by user: {update.effective_user.id}, error: {error_msg}"
        )
...
async def handle_currency(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not is_valid:
        logger.warning(
            f"Invalid currency input by user: {update.effective_user.id}, error: {error_msg}"
        )
        await update.message.reply_text(error_msg)
        return CreateGroupStates.CURRENCY
    ...
    try:
        ...
        if not group_data or "name" not in group_data or "default_currency" not in group_data:
            await query.edit_message_text(
                "Missing required data. Please start again using /newgroup command."
            )
